...
/* Mongoose is bad in calculating of SHA1 on big endian systems. Use sha1 functions from sha1sum */
#include "sha1sum.h"
...
static void ws_handshake(struct mg_connection *nc, const struct mg_str *key) {
  ...
  char buf[MG_VPRINTF_BUFFER_SIZE], b64_sha[sizeof(sha) * 2];
//  cs_sha1_ctx sha_ctx;
  
//  printf("Key: %.*s\n", (int) key->len, key->p);

  snprintf(buf, sizeof(buf), "%.*s258EAFA5-E914-47DA-95CA-C5AB0DC85B11", (int) key->len, key->p);

//   cs_sha1_init(&sha_ctx);
//   cs_sha1_update(&sha_ctx, (unsigned char *) buf, strlen(buf));
//   cs_sha1_final(sha, &sha_ctx);
//  mg_base64_encode(sha, sizeof(sha), b64_sha);

  SHA1_CONTEXT ctx;
  sha1_init(&ctx);
  sha1_write(&ctx, (unsigned char*)(buf), strlen(buf));
  sha1_final(&ctx);

  mg_base64_encode(ctx.buf, sizeof(sha), b64_sha);
