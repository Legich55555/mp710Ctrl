...
void OnDeviceUpdate(bool result, const DeviceController::Command& command);
...
void Sunrise(const std::chrono::seconds& duration)
{
    ...
    const std::chrono::milliseconds stepDuration = durationMs / DeviceController::kBrightnessMax / CHANNELS.size();

    for (auto channel : CHANNELS) {
        LinearEasing(deviceController, channel, 0, DeviceController::kBrightnessMax, stepDuration);
    }
